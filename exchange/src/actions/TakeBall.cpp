...
#include <exchange/BallPosForHead.h>
#include <exchange/BallPosForHand.h>
...
void moveArm(int nao,std::string ip,float x,float y,float z,float a,float b,float c,std::string effector,AL::ALMotionProxy* motion_proxy_ptr){
	...
	int axisMask = 7; //remettre à 63 ou 7 au cas où
...
void placeArmForExchange(int nao,std::string ip,int arm,Eigen::Vector3f position,AL::ALMotionProxy* motion_proxy_ptr){
	...
	if (arm==1){//LEFT ARM
		...
		//set arm
		moveArm(nao,ip,position[0]-result[0],position[0]-result[0],position[0]-result[0],0,0,0,"LArm",motion_proxy_ptr);
	}
}
//////////
...
public:
	...
	int nao_p;
	std::string nao_id_p;
	exchange::BallPosForHead srv3;
	exchange::BallPosForHand srv4;
	exchange::ArmReadyBT srv6;
...
int main(int argc, char** argv)
{
	if(argc != 1)
	{
		...
		client3 = n.serviceClient<exchange::BallPosForHead::Response>(headsrv);
		client4 = n.serviceClient<exchange::BallPosForHand::Response>(handsrv);
		client6 = n.serviceClient<exchange::ArmReadyBT::Response>("armReady");
